	...
	: Base(device, desc)
{
	...
	{ // create command queue
		...
		hr = d3dDevice->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(m_commandQueue.ptrForInit()));
	...
	{ // create swap chain
		...
		swapChainDesc.Height				  = static_cast<int>(desc.window->clientRect().h);
	...
	#if 0
	hr = d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocator.Get(), nullptr, IID_PPV_ARGS(m_commandList.ptrForInit()));
	AXE_DX12_THROWIF_HRESULT_ERROR(hr, d3dDevice);
...
void Context_DX12::_test_LoadAssets()
{
	...
	hr = d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocator.Get(), m_pipelineState.Get(), IID_PPV_ARGS(m_commandList.ptrForInit()));
	AXE_DX12_THROWIF_HRESULT_ERROR(hr, d3dDevice);
	...
	{ // Create the vertex buffer.
		float m_aspectRatio = frameBufferSize().x / frameBufferSize().y;
...
void Context_DX12::_test_PopulateCommandList() {
	...
	m_viewport.Width	= frameBufferSize().x;
	m_viewport.Height	= frameBufferSize().y;
	...
	m_scissorRect.top	 = 0;
	m_scissorRect.right  = static_cast<LONG>(frameBufferSize().x);
	m_scissorRect.bottom = static_cast<LONG>(frameBufferSize().y);
...
void Context_DX12::_test_WaitForPreviousFrame() {
	...
	hr = m_commandQueue->Signal(m_fence.ptr(), curFenceValue);
	AXE_DX12_THROWIF_HRESULT_ERROR(hr, d3dDevice);
