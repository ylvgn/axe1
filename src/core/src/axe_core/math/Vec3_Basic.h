...
struct Vec3_Basic : public DATA {
	...
	static constexpr This s_zero()		{ return This( 0, 0, 0); }
	static constexpr This s_one()		{ return This( 1, 1, 1); }

	static constexpr This s_forward()	{ return This( 0, 0, 1); }
	static constexpr This s_back()		{ return This( 0, 0,-1); }
	static constexpr This s_up()		{ return This( 0, 1, 0); }
	static constexpr This s_down()		{ return This( 0,-1, 0); }
	static constexpr This s_right()		{ return This( 1 ,0, 0); }
	static constexpr This s_left()		{ return This(-1 ,0, 0); }

	static constexpr This s_xy0(const Vec2& v) { return This(v.x, v.y, T(0)); }
	static constexpr This s_xy1(const Vec2& v) { return This(v.x, v.y, T(1)); }
	static constexpr This s_x0y(const Vec2& v) { return This(v.x, T(0), v.y); }
	static constexpr This s_x1y(const Vec2& v) { return This(v.x, T(1), v.y); }

	static constexpr This s_inf()		{ auto v = Math::inf<T>(); return This(v); }
	...
	AXE_NODISCARD This	cross	(const This& v) const { return This(y*v.z - z*v.y, z*v.x - x*v.z,x*v.y - y*v.x); }
	AXE_NODISCARD T		dot		(const This& v) const { return (x*v.x) + (y*v.y) + (z*v.z); }

	AXE_NODISCARD T		magnitude		() const { T m = sqrMagnitude(); return Math::equals0(m) ? T(0) : Math::sqrt(m); }
	AXE_NODISCARD T		sqrMagnitude	() const { return dot(*this); }
	...
	AXE_NODISCARD constexpr Vec2 yx () const { return Vec2(y, x); }
	AXE_NODISCARD constexpr Vec2 yz () const { return Vec2(y, z); }
	AXE_NODISCARD constexpr Vec2 zx () const { return Vec2(z, x); }
	AXE_NODISCARD constexpr Vec2 zy () const { return Vec2(z, y); }

	AXE_NODISCARD constexpr Tuple3<T> toTuple() const { return Tuple3<T>(x, y, z); }

	AXE_NODISCARD constexpr This operator-() const { return This(-x, -y, -z); }

	AXE_NODISCARD This operator + (const This &	r) const { return This(x + r.x, y + r.y, z + r.z); }
	AXE_NODISCARD This operator - (const This &	r) const { return This(x - r.x, y - r.y, z - r.z); }
	AXE_NODISCARD This operator * (const This &	r) const { return This(x * r.x, y * r.y, z * r.z); }
	AXE_NODISCARD This operator / (const This &	r) const { return This(x / r.x, y / r.y, z / r.z); }

	AXE_NODISCARD This operator + (T s) const { return This(x + s, y + s, z + s); }
	AXE_NODISCARD This operator - (T s) const { return This(x - s, y - s, z - s); }
	AXE_NODISCARD This operator * (T s) const { return This(x * s, y * s, z * s); }
	AXE_NODISCARD This operator / (T s) const { return This(x / s, y / s, z / s); }

				  void operator += (const This & r) { x += r.x; y += r.y; z += r.z; }
				  void operator -= (const This & r) { x -= r.x; y -= r.y; z -= r.z; }
				  void operator *= (const This & r) { x *= r.x; y *= r.y; z *= r.z; }
				  void operator /= (const This & r) { x /= r.x; y /= r.y; z /= r.z; }
