...
void NativeUIWindow_Win32::onClientRectChanged(const Rect2f& rc) {
	if (!_hwnd) return;
...
void NativeUIWindow_Win32::onSetNativeWorldPos(const Vec2f& screenPos) {
	...
	::MoveWindow(_hwnd
		, out_screenPosPt.x
		, out_screenPosPt.y
		, static_cast<int>(rc.w)
		, static_cast<int>(rc.h)
		, FALSE
	);

	// after MoveWindow, os will default send 'WM_MOVE' message, so _worldRect.pos will refresh.
}
